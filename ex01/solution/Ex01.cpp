...
{
  /*/// Expectation of the ground truth
  double mu = ground_truth.avg();
  /// Variance of the ground truth
  double sigma_square = 0.;
  for ( int y = 0; y < ground_truth.ySize(); ++y ) {
    for ( int x = 0; x < ground_truth.xSize(); ++x ) {
      sigma_square += square(ground_truth(x,y) - mu);
    }
  }
  sigma_square /= (double)(ground_truth.xSize()*ground_truth.ySize());*/
...
void Ex01_a_GaussianNoise() 
{
  ...
  std::cout << "Noising Lena with standard deviation " << sigma 
            << " ...\n";
...
void Ex01_b_PSNR() 
{
  ...
  ground_truth.readFromPGM("lena.pgm");
  ...
  average.writeToPGM("lenaNoisyAverage.pgm");
}
             ...
             const std::vector<float>& filter1d)
{
  ...
  for (int y = 0; y < image.ySize(); ++y) {
    for (int x = 0; x < image.xSize(); ++x) {
      float response = 0.f;
      for (int i = -filter_offset; i <= filter_offset+1; ++i) {
        int _x = x+i;
